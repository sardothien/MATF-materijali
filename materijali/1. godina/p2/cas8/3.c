/* Sa standardnog ulaza se zadaje n, a zatim n realnih brojeva
dvostruke tacnosti. Kao argument komandne linije se zadaje opcija.
Napisati program koji u zavisnosti od zadate opcije sortira uneti
niz:
...
void insertion(int n, double *niz){ // pod a)
 int i, j;
 double tmp;
 for(i=1; i<n; i++){
  for(j=i; j>0; j--){
...
void selection(int n, double *niz){ // pod b)
 int i, j;
 int poz_min;
 double tmp;
 for(i=0; i<n-1; i++){
...
void merge(double *niz, int l, int d, double *pomocni){ // pod c)
 if(l>=d)
  return;
 int s=l+(d-l)/2;
...
void zamena(double *niz, int i, int j){ // za quick sort
 double tmp;
 if(i!=j){
  tmp=niz[i];
  niz[i]=niz[j];
...
void quicksort(double *niz, int l, int d){ // pod d)
 if(l>=d)
  return;
...
void insertionr(int n, double *niz){ // pod f)
 int i, j;
 double tmp;
 for(i=1; i<n; i++){
  for(j=i; j>0; j--){
...
void selectionr(int n, double *niz){ // pod g)
 int i, j;
 int poz_min;
 double tmp;
 for(i=0; i<n-1; i++){
...
void merger(double *niz, int l, int d, double *pomocni){ // pod h)
 if(l>=d)
  return;
 int s=l+(d-l)/2;
...
void quicksortr(double *niz, int l, int d){ // pod i)
 if(l>=d)
  return;
...
int main(int argc, char *argv[]){
 ...
 if(strcmp(argv[1], "-i")==0)
  ...
  qsort(niz, n, sizeof(double), poredi); 
